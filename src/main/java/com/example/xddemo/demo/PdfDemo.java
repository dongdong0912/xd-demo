...
//        ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, signaturePhrase, 170f, 1115, 0);
     ...
     * @throws DocumentException
     */
    public static void manipulatePdf222(String src, String dest) throws IOException, DocumentException {
        PdfReader reader = new PdfReader(src);
        PdfStamper stamper = new PdfStamper(reader, new FileOutputStream(dest));
        //添加一个遮挡处，可以把原内容遮住，后面在上面写入内容
        PdfContentByte canvas = stamper.getOverContent(1);  //可以遮挡文字
    ...
    public static void manipulatePdf1(String src, String dest) throws IOException, DocumentException {
        ...
        // 使用BaseFont创建字体对象
        BaseFont baseFont = BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
        Phrase signaturePhrase = new Phrase("天津滨海新区北塘街欣嘉园蓝卡社区卫生服务中心测试", new Font(baseFont, 15, Font.BOLD, BaseColor.BLACK));
    ...
    public static void test5() {
        ...
        try {
            ...
            // 使用BaseFont创建字体对象
            BaseFont baseFont = BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
            Phrase signaturePhrase = new Phrase("专家阅片结论", new Font(baseFont, 12, Font.BOLD, BaseColor.BLACK));
